...
/**
 * Performance test for the SiemLog entity.
 */
class SiemLogGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SiemLog entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all siemLogs")
            .get("/siemcenter/api/siem-logs")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new siemLog")
            .post("/siemcenter/api/siem-logs")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "operatingSystem":"WINDOWS"
                , "operatingSystemVersion":"SAMPLE_TEXT"
                , "internalIp":"SAMPLE_TEXT"
                , "externalIp":"SAMPLE_TEXT"
                , "hostName":"SAMPLE_TEXT"
                , "context":"SAMPLE_TEXT"
                , "message":"SAMPLE_TEXT"
                ...
                , "rawMessage":"SAMPLE_TEXT"
                }""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_siemLog_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created siemLog")
                .get("/siemcenter${new_siemLog_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created siemLog")
            .delete("/siemcenter${new_siemLog_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
