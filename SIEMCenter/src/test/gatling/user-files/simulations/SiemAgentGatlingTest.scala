...
/**
 * Performance test for the SiemAgent entity.
 */
class SiemAgentGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SiemAgent entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all siemAgents")
            .get("/siemcenter/api/siem-agents")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new siemAgent")
            .post("/siemcenter/api/siem-agents")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "cn":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_siemAgent_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created siemAgent")
                .get("/siemcenter${new_siemAgent_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created siemAgent")
            .delete("/siemcenter${new_siemAgent_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
