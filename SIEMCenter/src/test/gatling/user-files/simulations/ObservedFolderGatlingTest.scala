...
/**
 * Performance test for the ObservedFolder entity.
 */
class ObservedFolderGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ObservedFolder entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all observedFolders")
            .get("/siemcenter/api/observed-folders")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new observedFolder")
            .post("/siemcenter/api/observed-folders")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "path":"SAMPLE_TEXT"
                , "description":"SAMPLE_TEXT"
                , "filterRegex":"SAMPLE_TEXT"
                }""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_observedFolder_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created observedFolder")
                .get("/siemcenter${new_observedFolder_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created observedFolder")
            .delete("/siemcenter${new_observedFolder_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
