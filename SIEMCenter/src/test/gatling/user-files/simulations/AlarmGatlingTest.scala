...
/**
 * Performance test for the Alarm entity.
 */
class AlarmGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Alarm entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all alarms")
            .get("/siemcenter/api/alarms")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new alarm")
            .post("/siemcenter/api/alarms")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "message":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_alarm_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created alarm")
                .get("/siemcenter${new_alarm_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created alarm")
            .delete("/siemcenter${new_alarm_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
