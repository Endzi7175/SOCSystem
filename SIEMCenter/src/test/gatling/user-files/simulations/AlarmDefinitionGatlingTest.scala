...
/**
 * Performance test for the AlarmDefinition entity.
 */
class AlarmDefinitionGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the AlarmDefinition entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all alarmDefinitions")
            .get("/siemcenter/api/alarm-definitions")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new alarmDefinition")
            .post("/siemcenter/api/alarm-definitions")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_alarmDefinition_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created alarmDefinition")
                .get("/siemcenter${new_alarmDefinition_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created alarmDefinition")
            .delete("/siemcenter${new_alarmDefinition_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
